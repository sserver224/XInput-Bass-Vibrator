...
If you're using a DualShock 3 or a SixAxis controller wirelessly, bluetooth drivers such as DSHidMini + BthPS3 may impose a rate limit of ~6 packets per second by default (which means the rumble can only update at most ~6 times per second, and this is to prevent packet flooding), which may prevent low frequency pulses (such as bass drums) from being picked up from the software.
